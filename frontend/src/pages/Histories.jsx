...
export default function Histories() {
  ...
  const handleDeleteHistory = async () => {
    ...
    try {
      ...
      setError("");
      await apiDelete(`/clinical-histories/${historyToDelete.id}/`);
  ...
  return (
    <main className="container-pad py-14">
        ...
        <div className="space-y-4">
          {histories.map((history) => (
            <div key={history.id} className="card">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-slate-500">
                    {history.visit_date} · {history.vet_name}
                  </p>
                </div>
                <div className="flex items-center gap-2">
                  <span className="rounded-full bg-river/10 px-3 py-1 text-xs font-semibold text-river">
                    Historial
                  </span>
                  <button
                    type="button"
                    onClick={() => setHistoryToDelete(history)}
              ...
              {history.notes && (
                <p className="mt-2 text-sm text-slate-500">{history.notes}</p>
              )}
            </div>
          ))}
          {histories.length === 0 && (
      ...
      <ConfirmModal
        ...
        message={
          historyToDelete
            ? `Se eliminará la historia clínica de ${historyToDelete.pet_name} registrada el ${historyToDelete.visit_date}.`
            : ""
        }
